...
COLOR_FOONATHAN = '#a855f7'  # Vibrant Purple (foonathan::memory)
COLOR_WB        = '#8c92ac'  # Slate Grey (wb_alloc)

# Background & Card Styling (Solid dark container for 100% visibility on light/dark themes)
BG_COLOR    = '#181820'     # Sleek dark charcoal background
LEGEND_BG   = '#222530'     # Dark grey background for legend
TEXT_COLOR  = '#f0f0f5'     # Crisp light text
GRID_COLOR  = '#333745'     # Subtle grid lines
...
def generate_throughput_chart(filename, title, y_min, y_max, y_step,
                              ...
                              throughput_obstack=None, throughput_foonathan=None):
    
    fig, ax = plt.subplots(figsize=(9.5, 5.8), facecolor=BG_COLOR)
    ax.set_facecolor(BG_COLOR)

    # Plot clean lines with distinct styles and markers representing all allocators
    ...
    ax.plot(depths, throughput_foonathan, marker='D', markersize=5, linewidth=1.5, linestyle='-.',
            color=COLOR_FOONATHAN, label='foonathan::memory (C++ Memory Stack)')
    ...
    # Legend configuration with dark grey background box
    legend = ax.legend(facecolor=LEGEND_BG, edgecolor=GRID_COLOR, fontsize=8.5, 
                       loc='upper right', labelspacing=0.8, handletextpad=1.0, framealpha=0.95)
    ...
    plt.tight_layout()
    plt.savefig(filename, dpi=300, facecolor=BG_COLOR, edgecolor='none')
    plt.close()

# ==============================================================================================
#  CHART 1: Throughput with Small Payloads (16-128 Bytes)
# ==============================================================================================
generate_throughput_chart(
    ...
    y_max=750,
    y_step=100,
    throughput_contract=np.array([553.95, 520.37, 497.31]),
    throughput_defensive=np.array([438.24, 425.04, 393.81]),
    throughput_obstack=np.array([492.32, 472.35, 354.83]),
    throughput_foonathan=np.array([418.05, 385.99, 406.24]),
    throughput_trebi=np.array([402.27, 399.90, 397.59]),
    throughput_wb=np.array([225.39, 228.38, 250.47])
)

# ==============================================================================================
#  CHART 2: Throughput with Large Payloads (512-4096 Bytes)
# ==============================================================================================
generate_throughput_chart(
    ...
    y_max=750,
    y_step=75,
    throughput_contract=np.array([586.84, 509.05, 498.43]),
    throughput_defensive=np.array([441.73, 395.46, 427.61]),
    throughput_foonathan=np.array([428.22, 380.73, 402.30]),
    throughput_trebi=np.array([403.17, 402.90, 397.68]),
    throughput_wb=np.array([73.77, 72.36, 72.36]),
    throughput_obstack=np.array([61.38, 58.90, 58.93])
)
...
ax.plot(alloc_sizes, inline_efficiency, marker='s', markersize=6, linewidth=2.5,
        color='#5d6470', label='Traditional Allocator (16-byte Inline Header)')
...
for i, sz in enumerate(alloc_sizes):
    ...
    ax.text(x_text, y_text, badge_text, 
            ...
            va='center', ha='left',
            bbox=dict(boxstyle='round,pad=0.25', facecolor=LEGEND_BG, edgecolor='none', alpha=0.95))
...
legend = ax.legend(facecolor=LEGEND_BG, edgecolor=GRID_COLOR, fontsize=8.5, loc='lower left', framealpha=0.95)
for text in legend.get_texts():
    text.set_color(TEXT_COLOR)
...
plt.tight_layout()
plt.savefig('memory_efficiency_chart.png', dpi=300, facecolor=BG_COLOR, edgecolor='none')
plt.close()

print("All charts updated with solid dark background cards! 100% visible everywhere.")